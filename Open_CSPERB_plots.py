...
def tower_receiver_plots(files, efficiency=True, maps_3D=True, flux_map=True, flow_paths=True, saveloc=None, billboard=False, flux_limits_file=None):
	...
	if flow_paths:
		...
		fig = plt.figure(figsize=(4,2.+(len(fp)-1)*1.1), dpi=1000)
	
		plt.subplots_adjust(left=0.15, bottom=bot, right=0.95, top = top)
		for f in xrange(len(fp)):
			plt.subplot(len(fp),1,f+1)
			if len(fp)>1:
				plt.text(x=bank_lengths[-5], y=980, s='Flow path %s'%str(f+1), ha='right')
			bank_lengths = pipe_lengths[f]
			bank_lengths_2 = (bank_lengths[1:]+bank_lengths[:-1])/2.

			plt.plot(bank_lengths_2, flux_in[f]/1e3, label=r'${\dot{q}_\mathrm{in}^{\prime \prime}}$', color='0')
			plt.plot(bank_lengths_2, q_net[fp[f]]/areas[fp[f]]/1e3, label=r'${\dot{q}^{\prime \prime}_\mathrm{abs}}$', color='0.6')
			flux_lims = flux_limits(N.ones(len(fp[f]))/n_tubes[fp[f]]*m[f], T_HC[f], flux_limits_file)/1e3
			print N.amax(flux_in[f]/1e3)
			plt.plot(bank_lengths, flux_lims, color='r')
			plt.yticks(N.arange(-200,1600, 200))
		...
		fig = plt.figure(figsize=(6,2.+(len(fp)-1)*1.1), dpi=1000)
		plt.subplots_adjust(left=0.10, bottom=bot, right=0.95, top = top)

		for f in xrange(len(fp)):
			plt.subplot(len(fp),1,f+1)
			if len(fp)>1:
				plt.text(x=1, y=-5, s='Flow path %s'%str(f+1), va='bottom')
			bank_lengths = pipe_lengths[f]
			bank_lengths_2 = (bank_lengths[1:]+bank_lengths[:-1])/2.
			plt.plot(bank_lengths_2, -q_ref[fp[f]]/areas[fp[f]]/1e3, label=r'${\dot{q}^{\prime \prime}_\mathrm{ref}}$', color='0.6')
			plt.plot(bank_lengths_2, -q_conv[fp[f]]/areas[fp[f]]/1e3, label=r'${\dot{q}^{\prime \prime}_\mathrm{conv}}$', color='0.4')
			plt.plot(bank_lengths_2, -q_rad[fp[f]]/areas[fp[f]]/1e3, label=r'${\dot{q}^{\prime \prime}_\mathrm{rad}}$', color='0')
		...
		plt.savefig(open(saveloc+'_losses_fp.png','w'), dpi=400)
		plt.close(fig)

		fig = plt.figure(figsize=(6,2.+(len(fp)-1)*1.1), dpi=1000)
		plt.subplots_adjust(left=0.10, bottom=bot, right=0.90, top = 0.95)
		...
		for f in xrange(len(fp)):
			...
			bank_lengths = pipe_lengths[f]
			bank_lengths_2 = (bank_lengths[1:]+bank_lengths[:-1])/2.
			plt.plot(bank_lengths_2, V[fp[f]], color='0.5')
			plt.annotate("", xy=(0, V[fp[f]][len(fp[f])/2]), xytext=(bank_lengths_2[len(fp[f])/2], V[fp[f]][len(fp[f])/2]), arrowprops=dict(arrowstyle="->", color='0.5'))
			#plt.arrow(x=bank_lengths_2[100], y=V[fp[f]][100], dx=-bank_lengths_2[100], dy=0, color='0.5', length_includes_head=True, lw=0.5)
			plt.ylabel('HC velocity [m.s$^{-1}$]')
			plt.xlabel('Flow path length [m]')
			plt.twinx()

			Dploc = -N.add.accumulate(Dp[fp[f]])/1e5
			if len(fp)>1:
				plt.text(x=18, y=Dploc[20], s='Flow path %s'%str(f+1), va='top')
			plt.plot(bank_lengths_2, Dploc, color='0')
			plt.annotate("", xy=(bank_lengths_2[-1], Dploc[-len(fp[f])/2]), xytext=(bank_lengths_2[-len(fp[f])/2], Dploc[-len(fp[f])/2]), arrowprops=dict(arrowstyle="->"), color='0')
			#plt.arrow(x=bank_lengths_2[-100], y=Dploc[-100], dx=bank_lengths_2[-1]-bank_lengths_2[-100], dy=0, color='00', length_includes_head=True, lw=0.5)
			plt.ylabel('Pressure drop [bar]', rotation=-90, va='bottom')

		#plt.legend(loc=1)

		plt.xlabel('Flow path length [m]')

		plt.savefig(open(saveloc+'_p_drops.png','w'), dpi=400)
		plt.close(fig)	
		...
		for f in xrange(len(fp)):
			...
			bank_lengths = pipe_lengths[f]
			bank_lengths_2 = (bank_lengths[1:]+bank_lengths[:-1])/2.
			plt.plot(bank_lengths_2, h_conv_int[fp[f]], color='k', lw=1)

			#plt.hlines(y=0., xmin=0., xmax=N.amax(bank_lengths_2), lw=0.5)
			plt.ylabel('h [W.m$^{-2}$.K$^{-1}$]')
			if f==0:
				plt.legend(loc=3,ncol=3, borderaxespad=0, bbox_to_anchor=(0.,1.05))

		plt.xlabel('Flow path length [m]')

		plt.savefig(open(saveloc+'_h_int.png','w'), dpi=400)
		plt.close(fig)
